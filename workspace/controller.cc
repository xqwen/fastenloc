	...
	for (int i = 0; i < eqtl_vec.size(); i++)
	{
	    ...
	    */
		...
		for (int k = 0; k < eqtl_vec[i].snp_vec.size(); k++){
			...
			double p11 = (pi1_e/pi1)*eqtl_vec[i].pip_vec[k]*gprob_vec_m[k];
			double p10 = (pi1_ne/pi1)*gprob_vec_m[k]*(1-eqtl_vec[i].pip_vec[k]);
			double p01 = ((1-pi1_e)/(1-pi1))*(1-gprob_vec_m[k])*eqtl_vec[i].pip_vec[k];
			double p00 = ((1-pi1_ne)/(1-pi1))*(1-gprob_vec_m[k])*(1-eqtl_vec[i].pip_vec[k]);


			RCP2 += p11/(p11+p10+p01+p00);
		...
		{
			string locus_id = eqtl_vec[i].id + "(@)" + snp2gwas_locus[max_snp];
			fprintf(fd2, "%15s   %4d  %7.3e %7.3e    %7.3e      %7.3e\t%7.3e\t\t\t%7.3e\n", locus_id.c_str(), int(eqtl_vec[i].snp_vec.size()), eqtl_vec[i].cpip, gprob_cpip_m, gprob_cpip_e, RCP, LCP, RCP2);
		}
	}
