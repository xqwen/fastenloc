...
cat(paste0("GWAS (FDR 5%):   Realized FDR ", round(gwas_fdr,3), "   Power ", round(gwas_power,3), " (", length(which(d_gwas$V2>=gwas_thresh & d_gwas$V3 == 1)),"/",gwas_total,")"),'\n')
...
cat(paste0("eQTL (FDR 5%):   Realized FDR ", round(eqtl_fdr,3), "   Power ", round(eqtl_power,3), " (", length(which(d_eqtl$V2>=eqtl_thresh & d_eqtl$V3 == 1)),"/",eqtl_total,")"), '\n')
